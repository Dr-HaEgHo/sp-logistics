...
const buttons2 = [
  ...
  { id: 9, name: "Proof Of Receipt", icon: "" },
  { id: 9, name: "Proof Of Delivery", icon: "" },
];
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const buttons = [
    ...
    { id: 3, name: "Create Quotation", icon: "" },
    {
      ...
      cta: () => setOpenReceipt(true)
    },
    {
      id: 5,
      name: "Proof of Delivery",
      icon: require("@/assets/icons/docu-sec700.svg"),
      cta: () => setOpenDeliveryProof(true)
    },
    {
      id: 6,
      ...
      <div className="w-full p-10">
        <ul className="flex flex-wrap gap-6">
          ...
          <li className="">
            <FilledButton
              text="Save"
              image={require("../../../../assets/icons/download.svg")}
            ...
            buttons2.map((btn) => (
              <li className="">
                <FilledButton
                  ...
                  pClass={`text-grey700 text-sm font-semibold ${tab === btn?.name.toLowerCase().split(" ").join("-") && '!text-sec700' }`}
                  cta={() => {
                    router.push(
                      `?tab=${btn.name.toLowerCase().split(" ").join("-")}`
                    );
