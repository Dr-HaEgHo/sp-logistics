...
const buttons = [
  ...
  { id: 4, name: "Add documents/Notes", icon: require('@/assets/icons/docu-sec700.svg')},
  { id: 5, name: "Take Action", icon: require('@/assets/icons/take-action-sec700.svg')},
//   { id: 6, name: "Print", icon: require('@/assets/icons/printer-sec700.svg')},
];

const buttons2 = [
  { id: 1, name: "Details",icon: ""},
  { id: 2, name: "Sales Invoices", count : '0' ,icon: ""},
  { id: 3, name: "Purchase Invoices", count : '0' ,icon: ""},
  { id: 4, name: "Quotation", count : '0' ,icon: ''},
  { id: 5, name: "Notes/Documents", count : '0' ,icon: ''},
  { id: 6, name: "Track Action", count : '0' ,icon: ''},
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const { setLayout, setAppHeaderInfo } = useContext(GlobalContext);
            ...
            buttons2.map((btn) => (
              <li className="">
                <FilledButton
                  ...
                  btnClass={`border-sec700 !rounded-none !py-[13.5px] p-[10px] ${tab === btn?.name.toLowerCase().split(' ').join('-') && 'border-b'} `}
                  pClass="text-sec700 text-sm font-semibold"
                  cta={() => {
                    router.push(`?tab=${btn.name.toLowerCase().split(' ').join('-')}`)
                  }}
                />
              </li>
            ))}
